...
####RseQC####
	for ($index=0;$index<$number;$index++){
		$nodeindex=$index % $nodes_number;
		print $data[$index],"\n";
        	my $rseqc_comman1 = "qsub -q all.q -hold_jid rsem_cal -pe smp 5 -N rseqc -cwd $cmd_path/rseqc.sh $organism $seq $data[$index]";
...
if($qc==1 && $genebody ==1){
	...
	execute("qsub -q all.q -hold_jid rsem_count_tmp_summary -N folder_build -cwd $cmd_path/mkdir.sh");
	my $cmd_email="qsub -q all.q -hold_jid folder_build -o ./node_log -e ./node_log -N organize -cwd $cmd_path/organize.sh";
	execute($cmd_email);
}
